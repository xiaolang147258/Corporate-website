<template>
  <!--品牌合作-->
  <div id="hello" style="width:100%;padding-bottom: 50px;overflow: hidden;">
    ...
    <div class="top_box">
    	<div class="top_box_c_box" :style="width>1100?'width:1331px;padding:0':(width<1100&&width>=720?'width:auto;padding:0 30px':(width<720?'width:100%;padding:0 10px 0 30px':''))" @yi-add>
    		<span><p class="left" v-for="(i,index) in tab" @click="p_click(i,index)" :class="{top_box_c_box_p:tab_show==index}" v-if="width<614?index<=2:index<=tab.length-1">{{i.title}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a>|</a></p></span>
            <span :style="width>800?'':'display:none'" @yi-add>
            	<p class="right_p" style="color:#b7d537;"><a>/</a>发声艺术介绍</p>
            	<p class="right_p"><a>/</a>品牌合作</p>
...
export default {
  ...
  methods:{
	 git_act(){
	 	axios.get('api/article/3').then(res=>{
	 		            // console.log(res);
	 		    	  if(res.code = 200){
